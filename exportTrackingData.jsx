...
function exportLayers(){
	if (layers.length == 0) {
		alert("No layers selected. Please select some layers with keyframes to export.");
		return;
	}
	...
	for (var p = 0; p < layers.length; p++) {
		...
		trackingData.layers[id] = { displayName: layers[p].name, numKeys: layers[p].Transform.Position.numKeys };
	}
	...
	for (var i = 0; i < layers.length; i++) {
		...
		for (var k = 1; k <= keyCount; k++) {
			...
			if (trackingData.frames[keyframe] === undefined) trackingData.frames[keyframe] = {};
			// save the position properties under the layer ID for this keyframe
			trackingData.frames[keyframe][layers[i].trackingID] = [ 	
				((layers[i].Transform.Position.valueAtTime(frameTime, true)[0]/composition.width)*100).toFixed(2) , // x%
				((layers[i].Transform.Position.valueAtTime(frameTime, true)[1]/composition.height)*100).toFixed(2)  // y%
			]
		}
	}
}
